...
class CadCodeRefinerChain(SequentialChain):
    ...
    def __init__(self, model_type: MODEL_TYPE = "gpt") -> None:
        refine_cad_code_prompt = (
            ...
            "If feedback says the code called `cutBlind`/`cutThruAll` from a fresh Workplane, rewrite that operation so it starts from the actual owning solid face or from an explicit cutter solid that is then subtracted from `result`. Do not keep absolute-offset construction planes for boolean cuts.\n"
            "For holes on a recessed floor, prefer `result.faces(...).workplane().polarArray(...).hole(...)` or a face-attached cut profile. Do not use `result.workplane(offset=...)...cutBlind(...)` as the final repair.\n"
            ...
            "If feedback says a required fillet/chamfer was hidden behind `try/except` or a silent `pass`, remove that fallback and replace it with a construction that deterministically produces the required feature.\n"
