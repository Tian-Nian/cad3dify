...
class CadCodeFromJsonGeneratorChain(SequentialChain):
    ...
    def __init__(self, model_type: MODEL_TYPE = "gpt") -> None:
        ...
        gen_cad_code_prompt = (
            ...
            "* Never start a subtractive operation from a fresh or detached construction workplane. Do not call `cutBlind()`/`cutThruAll()` from `cq.Workplane(\"XY\")...`, from `result.workplane(offset=...)`, or from any workplane that is not first attached to the owning solid face via `result.faces(...).workplane()`.\n"
            "* For hole arrays or pockets on an internal floor, first select the real owning face on `result` such as `result.faces(\">Z[1]\").workplane()` or build an explicit cutter solid and subtract it from `result`. Do not sketch at an absolute Z offset and assume the solid relationship is preserved.\n"
            "* If you use `polarArray`/`pushPoints` for cuts, the chain must still originate from the target solid's face/workplane, not from a standalone construction plane.\n"
            ...
            "* Never return code that catches a required fillet/chamfer failure and continues with `pass`, `continue without fillet`, or a similar silent fallback. Return one strategy that either robustly models the round or rebuilds the profile to include it.\n"
